...
class _Unchanged:
    ...
    A plain ``object()`` works just as well for the ``is`` comparisons, but prints
    as ``<object object at 0x104f2a310>`` in every signature that help() and an
    editor tooltip show.
    """
...
class WorksheetToolkit:
    ...
    def __init__(self, worksheet):
        ...
        ------
        TypeError
            If given anything other than a worksheet. Passing the workbook is an
            easy slip, and without this it surfaces much later as an
            AttributeError from inside a private method.
        """
    ...
    def unmerge_cells(
        ...
        ------
        ValueError
            If the range is not currently merged. openpyxl raises this, and it is
            left alone: silently ignoring the call would hide a typo in the range.
        """
        self._check_range_arguments(
